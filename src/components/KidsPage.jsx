...
function Kids({allItems,item,modifyCart, cartItems, 
     ...
     <div className="banner">
      <img src='https://images.unsplash.com/photo-1556905055-8f358a7a47b2?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=870&q=80' alt="" />
       ...
       <span>Kids</span>
        ...
        <div className='navigation mb-5 p-4'> <p className='container mx-auto flex items-center'>Home  <ArrowRight2 size="16"/>  Kids</p></div>
...
<div className="product-list gap-6 md:gap-1">
{
    allItems.map(item=>{
        if(item.category === 'Kids'){
            return <ItemCard item={item} updateLike={updateLike}
             key={item.id} modifyCart={modifyCart} setProductPage={setProductPage} productPage={productPage}/>
        }
})
}
</div>
